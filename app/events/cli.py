# TODO pyarrow
# TODO work on the fast api responses
# TODO implement transactions dataset
# TODO implement yelp dataset
# TODO enable avro for [movies]
# TODO gcs, adls2, s3
...
from datastore.confluentcloud.kafka_json import ConfluentCloud
...
from datastore.storage.stg_blob_storage import BlobStorage
from datastore.skin.events_skin import Skin
from datastore.cosmosdb.sql import CosmosDB
from datastore.eventhubs.eventhubs_json import EventHubs
from datastore.sqldb.azure_sql_db import AzureSQLDB
from datastore.yugabytedb.ysql import YSQL
from datastore.yugabytedb.ycql import YCQL
from datastore.pubsub.google_pub_sub import PubSub
from datastore.pulsar.pulsar_json import Pulsar
...
if __name__ == '__main__':
    ...
    parser.add_argument('entity', type=str, choices=[
        ...
        'strimzi-musics-avro',
        'strimzi-rides-avro',
        'strimzi-user-events-json',
        'strimzi-flight-events-json',
        'confluent-cloud-users-json',
        'confluent-cloud-agent-json',
        'confluent-cloud-musics-json',
        'confluent-cloud-rides-json',
        'pulsar-rides-json',
    ...
    get_user_events = Events().get_user_events(get_dt_rows)
    get_flight_events = Events().get_flight_events(get_dt_rows)
    ...
    if sys.argv[1] == 'skin':
        Skin().generate_events()

    if sys.argv[1] == 'strimzi-user-events-json':
        Kafka().json_producer(broker=kafka_broker, object_name=get_user_events,kafka_topic=user_events_json_topic)

    if sys.argv[1] == 'strimzi-flight-events-json':
        Kafka().json_producer(broker=kafka_broker, object_name=get_flight_events,kafka_topic=flight_events_json_topic)

    # confluent cloud
    if sys.argv[1] == 'confluent-cloud-users-json':
        ConfluentCloud().json_producer(broker=confluent_cloud_kafka_broker, object_name=users_object_name, kafka_topic=users_json_topic)

    if sys.argv[1] == 'confluent-cloud-agent-json':
        ConfluentCloud().json_producer(broker=confluent_cloud_kafka_broker, object_name=agent_object_name, kafka_topic=agent_json_topic)

    if sys.argv[1] == 'confluent-cloud-musics-json':
        ConfluentCloud().json_producer(broker=confluent_cloud_kafka_broker, object_name=musics_object_name, kafka_topic=musics_json_topic)

    if sys.argv[1] == 'confluent-cloud-rides-json':
        ConfluentCloud().json_producer(broker=confluent_cloud_kafka_broker, object_name=rides_object_name, kafka_topic=rides_json_topic)

    # apache pulsar
    if sys.argv[1] == 'pulsar-rides-json':
        Pulsar().json_producer(object_name=rides_object_name)
    ...
    if sys.argv[1] == 'blob-storage':
        BlobStorage().write_all()