...
class AzureSQLDB(object):
    ...
    def insert_rows():
        ...
        azure_sql_db_engine = create_engine(sqldb)

        pd_df_bank.to_sql('bank', azure_sql_db_engine, if_exists='append', index=False, chunksize=10)
        pd_df_credit_card.to_sql('credit_card', azure_sql_db_engine, if_exists='append', index=False, chunksize=10)
        pd_df_subscription.to_sql('subscription', azure_sql_db_engine, if_exists='append', index=False, chunksize=10)
        pd_df_company.to_sql('company', azure_sql_db_engine, if_exists='append', index=False, chunksize=10)
